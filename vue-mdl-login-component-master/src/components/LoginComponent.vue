    ...
    <div class="mdl-card mdl-shadow--4dp my-login-card-size">
      <form @submit.prevent="logMeIn">
        ...
        <div class="mdl-card__actions mdl-card--border">
          ...
          <span v-if="errorMessage" class="mdl-color-text--red-500">
...
export default {
  ...
  mounted: function() {
    let email = document.cookie.match(
      "(^|;)\\s*" + "email" + "\\s*=\\s*([^;]+)"
    );
    ...
    console.log(
      "We just check to see if there were cookies: " + document.cookie
    );
  },
  methods: {
    ...
    checkPasswordValidator: function() {
      if (this.$refs.txtPassword.checkValidity()) {
        if (
          this.$refs.txtEmail.checkValidity() &&
    ...
    logMeIn: function() {
      ...
      if (this.rememberMe) {
        ...
        document.cookie =
          "email=" + email + ";expires=" + d.toUTCString() + ";path=/";
        document.cookie =
